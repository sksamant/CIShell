...
public class AlgorithmAction extends Action implements AlgorithmProperty,
		DataManagerListener {
	...
	public AlgorithmAction(ServiceReference serviceReference,
			BundleContext bundleContext, CIShellContext ciShellContext) {
		this((String) serviceReference.getProperty(LABEL), serviceReference,
				bundleContext, ciShellContext);
	}

	public AlgorithmAction(String label, ServiceReference serviceReference,
			BundleContext bundleContext, CIShellContext ciShellContext) {
		...
		setText(label);
		setToolTipText((String) serviceReference
				.getProperty(AlgorithmProperty.DESCRIPTION));
	...
	public void run() {
		try {
			...
			// Just in case an uncaught exception occurs. Eclipse will swallow
			// errors thrown here.
	...
	private void printAlgorithmInformation(ServiceReference serviceReference, CIShellContext ciContext) {
		...
		LogService logger = (LogService) ciContext
		...
		String implementers = (String) serviceReference
		...
		String reference_url = (String) serviceReference
				.getProperty(REFERENCE_URL);

		if ((reference != null) && (reference_url != null)) {
			acknowledgement.append("Reference: " + reference + " ([url]"
					+ reference_url + "[/url])\n");
		} else if ((reference != null) && (reference_url == null)) {
			acknowledgement.append("Reference: " + reference + "\n");
		}

		String documentationURL = (String) serviceReference
				.getProperty(DOCUMENTATION_URL);

		if (documentationURL != null) {
			acknowledgement.append("Documentation: [url]" + documentationURL
					+ "[/url]\n");
		}

		if (acknowledgement.length() > 1) {
			logger.log(serviceReference, LogService.LOG_INFO,
	...
	public void dataSelected(Data[] selectedData) {
		...
		if ((inData.length == 1) && inData[0].equalsIgnoreCase(NULL_DATA)) {
			...
			DataConversionService converter = (DataConversionService) this.ciShellContext
			...
			if(dataSet.size() == 0){
				...
				this.data = new Data[dataSet.size()];
				this.converters = new Converter[dataSet.size()][];
